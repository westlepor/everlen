/*
 * This test is meant to check a multiple client admin with access codes logging in does not have
 * super admin features/access.
 */

describe("logging in as multiple client admin with access codes permission", () => {
  before(function () {
    cy.login(this.testData.users.multiple_client_admin_access_codes)
  })
  ...
  specify("profile has correct name and company", function () {
    ...
    cy.get("div[data-cy=user-profile]").should(
      "contain",
      this.testData.users.multiple_client_admin_access_codes.name
    )
    cy.get("div[data-cy=user-profile]").should("contain", "Company")
    this.testData.users.multiple_client_admin_access_codes.companies.forEach(
      company => {
        cy.get("div[data-cy=user-profile]").should("contain", company)
      }
    )
