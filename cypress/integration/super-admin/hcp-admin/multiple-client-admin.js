 ...
 * masquerading with a HCP multiple client admin
 */
import {
  HCP_COLUMN_HEADERS,
...
describe("logging in as super admin and masquerade with HCP mutliple client admin", () => {
  before(function () {
    cy.login(this.testData.users.super_admin)
  })

  specify("it can masquerade HCP multiple client admin", function () {
    cy.get("div[data-cy=super-admin-banner]")
      .find("input")
      .type(this.testData.users.multiple_client_admin_hcp.username)
    cy.get("div[data-cy=super-admin-banner]")
