 ...
 * masquerading with a multiple client admin
 */

describe("logging in as super admin and masquerade with mutliple client admin", () => {
  before(function () {
    cy.login(this.testData.users.super_admin)
  })

  specify("it can masquerade multiple client admin", function () {
    cy.get("div[data-cy=super-admin-banner]")
      .find("input")
      .type(this.testData.users.multiple_client_admin.username)
    cy.get("div[data-cy=super-admin-banner]")
