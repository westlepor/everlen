 ...
 * masquerading with a client admin access codes
 */

describe("logging in as super admin and masquerade with client admin access codes", () => {
  before(function () {
    cy.login(this.testData.users.super_admin)
  })

  specify("it can masquerade client admin with access codes", function () {
    cy.get("div[data-cy=super-admin-banner]")
      .find("input")
      .type(this.testData.users.client_admin_access_codes.username)
    cy.get("div[data-cy=super-admin-banner]")
